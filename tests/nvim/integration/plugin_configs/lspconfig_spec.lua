describe("sumneko_lua", function()
  ...
  describe("root_dir()", function()
    ...
    it("prioritizes stylua.toml over lua directory", function()
      ...
      assert.are.equal(
        current_dir,
        sumneko_lua.root_dir(vim.loop.fs_realpath("nvim/lua/utils.lua"))
      )
    end)
  end)
  ...
  describe("on_new_config()", function()
    ...
    it("works for dotfiles", function()
      ...
      assert.matches("luassert", new_config.settings.Lua.workspace.library[1], 1, true)
      assert.matches("plenary.nvim", new_config.settings.Lua.workspace.library[2], 1, true)
      assert.are.equal("${3rd}/busted", new_config.settings.Lua.workspace.library[3])
      assert.are.equal("${3rd}/luassert", new_config.settings.Lua.workspace.library[4])
      assert.matches("lua/mini/test.lua", new_config.settings.Lua.workspace.library[5], 1, true)
      assert.matches("types/stable$", new_config.settings.Lua.workspace.library[6])
      assert.are.equal(vim.env.VIMRUNTIME, new_config.settings.Lua.workspace.library[7])
      assert.are.equal(7, #new_config.settings.Lua.workspace.library)
    end)
  end)
end)
